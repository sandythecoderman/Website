...
function initPCEyes() {
    ...
    // Only initialize if we're on a page with PC eyes elements
    if (pcContainer && leftPupil && rightPupil) {
        console.log('PC Eyes Init:', { pcContainer, leftPupil, rightPupil });
        
        // Enable eye tracking immediately on page load
        isEyeTracking = true;
        console.log('Eye tracking enabled on page load');
        ...
        // Silently skip initialization on pages without PC eyes
        console.log('PC Eyes not available on this page');
    }
}
...
function initDynamicGraph() {
    ...
    if (graphNodes.length > 0) {
        ...
        console.log('Dynamic graph not available on this page');
    }
}
